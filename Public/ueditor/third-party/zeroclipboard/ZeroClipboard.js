...
(function(window) {
  ...
  var _determineScriptAccess = function() {
    var _extractAllDomains = function(origins, ressArray) {
      var i, len, tmp;
      if (origins == null || ressArray[0] === "*") {
        return;
      }
  ...
  var _mapClipressFromFlash = function(clipress, formatMap) {
    if (!(typeof clipress === "object" && clipress && typeof formatMap === "object" && formatMap)) {
      return clipress;
    }
    var newress = {};
    for (var prop in clipress) {
      if (clipress.hasOwnProperty(prop)) {
        if (prop !== "success" && prop !== "data") {
          newress[prop] = clipress[prop];
          continue;
        }
        newress[prop] = {};
        var tmpHash = clipress[prop];
        for (var dataFormat in tmpHash) {
          if (dataFormat && tmpHash.hasOwnProperty(dataFormat) && formatMap.hasOwnProperty(dataFormat)) {
            newress[prop][formatMap[dataFormat]] = tmpHash[dataFormat];
          }
        }
      }
    }
  ...
  var _createEvent = function(eventType, event) {
    ...
    if (event.type === "aftercopy") {
      event = _mapClipressFromFlash(event, _clipDataFormatMap);
    }
