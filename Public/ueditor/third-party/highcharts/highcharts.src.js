...
function format(str, ctx) {
	...
	while ((index = str.indexOf(splitter)) !== -1) {
		...
		if (isInside) { // we're on the closing bracket looking back
			...
			// Push the res and advance the cursor
			ret.push(val);
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		map: function (arr, fn) {
			...
			var ress = [],
				...
				len = arr.length;
			for (; i < len; i++) {
				ress[i] = fn.call(arr[i], arr[i], i, arr);
			}
			return ress;
...
var Color = function (input) {
	...
	function init(input) {
		...
		// Solid colors
			...
			// rgba
			res = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]?(?:\.[0-9]+)?)\s*\)/.exec(input);
			if (res) {
				rgba = [pInt(res[1]), pInt(res[2]), pInt(res[3]), parseFloat(res[4], 10)];
			} else { 
				// hex
				res = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(input);
				if (res) {
					rgba = [pInt(res[1], 16), pInt(res[2], 16), pInt(res[3], 16), 1];
				} else {
					// rgb
					res = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(input);
					if (res) {
						rgba = [pInt(res[1]), pInt(res[2]), pInt(res[3]), 1];
					}
				}
			}
		}		
...
SVGElement.prototype = {
	...
	attr: function (hash, val) {
		...
		// setter
			...
			for (key in hash) {
				...
				res = attrSetters[key] && attrSetters[key].call(wrapper, value, key);

				if (res !== false) {
					if (res !== UNDEFINED) {
						value = res; // the attribute setter has returned a new value to set
					}
	...
	*/
	 ...
	 * Issue 490; in VML removeChild ress in Orphaned nodes according to sIEve, discardElement does not.
	 */
...
Axis.prototype = {
	...
	getLogTickPositions: function (interval, min, max, minor) {
		var axis = this,
			...
			// use a local variable and return the res
			positions = []; 
