...
var Parser = P(function(_, _super, Parser) {
  ...
  //   var res = SomeParser.parse('Me Me Me! Parse Me!');
  ...
  _.parse = function(stream) {
    ...
    function success(stream, res) { return res; }
  };
  ...
  _.then = function(next) {
    ...
    return Parser(function(stream, onSuccess, onFailure) {
      ...
      function success(newStream, res) {
        var nextParser = (next instanceof Parser ? next : next(res));
  ...
  _.times = function(min, max) {
    ...
    return Parser(function(stream, onSuccess, onFailure) {
      ...
      for (var i = 0; i < min; i += 1) {
        res = self._(stream, success, firstFailure);
        if (!res) return onFailure(stream, failure);
      }

      for (; i < max && res; i += 1) {
        res = self._(stream, success, secondFailure);
      }
  ...
  _.res = function(res) { return this.then(succeed(res)); };
  ...
  _.map = function(fn) {
    return this.then(function(res) { return succeed(fn(res)); });
  };

  _.skip = function(two) {
    return this.then(function(res) { return two.res(res); });
  };
  ...
  var regex = this.regex = function(re) {
    ...
    return Parser(function(stream, onSuccess, onFailure) {
      ...
      if (match) {
        var res = match[0];
        return onSuccess(stream.slice(res.length), res);
      }
      else {
        return onFailure(stream, expected);
      }
  ...
  var succeed = Parser.succeed = function(res) {
    return Parser(function(stream, onSuccess) {
      return onSuccess(stream, res);
    });
  };
 ...
 *     you override the res of a constructor call
...
var RootTextBlock = P(MathBlock, function(_) {
  _.renderLatex = function(latex) {
    ...
    var escapedDollar = string('\\$').res('$');
    var textChar = escapedDollar.or(regex(/^[^$]/)).map(VanillaSymbol);
    var latexText = mathMode.or(textChar).many();
    var commands = latexText.skip(eof).or(all.res(false)).parse(latex);
...
var Cursor = P(Point, function(_) {
  ...
  _.writeLatex = function(latex) {
    ...
    var block = latexMathParser.skip(eof).or(all.res(false)).parse(latex);
