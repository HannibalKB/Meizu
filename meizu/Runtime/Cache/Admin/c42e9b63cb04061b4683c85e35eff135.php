...
<div class="pd-20">
  <div class="cl pd-5 bg-1 bk-gray">
    <span class="l">
      ...
      <a class="btn btn-primary radius" href="<?php echo U('admin/role/addrole');?>" ><i class="icon-plus"></i> 添加角色</a>
    </span>
             ...
             <tr class="text-c">
                ...
                <td>
                  <?php  foreach ($value['son'] as $k => $v) { ?>
                        <a onclick="user_show(<?php echo $v['uid'] ?>,'500','700','<?php echo $v['username'] ?>','<?php echo U('admin/user/usershow',array('userid'=>$v['uid']));?>')"><?php echo $v['username'] ?></a>
                      <?php
 } ?>
                </td>
                <td><?php echo $value['desc'] ?></td>
                <td class="f-14"><a title="编辑" href="<?php echo U('admin/role/editrole',array('id'=>$value['id']));?>"  style="text-decoration:none"><i class="icon-edit"></i></a> <a title="删除" href="javascript:;" onclick="role_dels(this,'<?php echo $value['id'] ?>')" class="ml-5" style="text-decoration:none"><i class="icon-trash"></i></a></td>
              </tr>
           <?php
 } ?>
...
<script type="text/javascript">
  ...
  function role_dels(obj,id){
    layer.confirm('确认要删除吗？',function(index){
      ...
      var roleid = id;
      $.ajax({
        url: '<?php echo U("admin/role/deleterole");?>',
        type: 'POST',
        dataType: 'json',
        data: {roleid: roleid},
