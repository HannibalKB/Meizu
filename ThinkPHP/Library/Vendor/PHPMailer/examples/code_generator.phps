...
try {
    if (isset($_POST["submit"]) && $_POST['submit'] == "Submit") {
        ...
        $example_code .= "\n\ntry {";
        $example_code .= "\n  \$mail->send();";
        $example_code .= "\n  \$ress_messages[] = \"Message has been sent using " .
        ...
        try {
            $mail->send();
            $ress_messages[] = "Message has been sent using " . strtoupper($_POST["test_type"]);
        } catch (phpmailerException $e) {
            throw new phpmailerAppException("Unable to send to: " . $to . ': ' . $e->getMessage());
        }
    }
} catch (phpmailerAppException $e) {
    $ress_messages[] = $e->errorMessage();
}
$example_code .= "\n}";
$example_code .= "\ncatch (phpmailerAppException \$e) {";
$example_code .= "\n  \$ress_messages[] = \$e->errorMessage();";
$example_code .= "\n}";
$example_code .= "\n\nif (count(\$ress_messages) > 0) {";
$example_code .= "\n  echo \"<h2>Run ress</h2>\\n\";";
$example_code .= "\n  echo \"<ul>\\n\";";
$example_code .= "\nforeach (\$ress_messages as \$res) {";
$example_code .= "\n  echo \"<li>\$res</li>\\n\";";
$example_code .= "\n}";
$example_code .= "\necho \"</ul>\\n\";";
$example_code .= "\n}";
