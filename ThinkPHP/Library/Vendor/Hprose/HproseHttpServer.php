...
class HproseHttpServer {
    ...
    */
    ...
    private function doInvoke() {
        ...
        do {
            ...
            if ($this->onAfterInvoke) {
                call_user_func($this->onAfterInvoke, $functionName, $args, $byref, $res);
            }
            // some service functions/methods may echo content, we need clean it
            ob_clean();
            if ($resMode == HproseresMode::RawWithEndTag) {
                $this->output->write($res);
                return;
            }
            elseif ($resMode == HproseresMode::Raw) {
                $this->output->write($res);
            }
            else {
                $this->output->write(HproseTags::Tagres);
                if ($resMode == HproseresMode::Serialized) {
                    $this->output->write($res);
                }
                else {
                    $writer->reset();
    ...
    public function addMissingFunction($function, $resMode = HproseresMode::Normal, $simple = NULL) {
        $this->addFunction($function, '*', $resMode, $simple);
    }
    public function addFunction($function, $alias = NULL, $resMode = HproseresMode::Normal, $simple = NULL) {
        if (is_callable($function)) {
            ...
            if (is_string($alias)) {
                ...
                $this->resModes[$aliasName] = $resMode;
    ...
    public function addFunctions($functions, $aliases = NULL, $resMode = HproseresMode::Normal, $simple = NULL) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($aliases_is_null) {
                $this->addFunction($function, NULL, $resMode, $simple);
            }
            else {
                $this->addFunction($function, $aliases[$i], $resMode, $simple);
            }
        }
    }
    public function addMethod($methodname, $belongto, $alias = NULL, $resMode = HproseresMode::Normal, $simple = NULL) {
        if ($alias === NULL) {
            $alias = $methodname;
        }
        if (is_string($belongto)) {
            $this->addFunction(array($belongto, $methodname), $alias, $resMode, $simple);
        }
        else {
            $this->addFunction(array(&$belongto, $methodname), $alias, $resMode, $simple);
        }
    }
    public function addMethods($methods, $belongto, $aliases = NULL, $resMode = HproseresMode::Normal, $simple = NULL) {
        $aliases_is_null = ($aliases === NULL);
        $count = count($methods);
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($aliases_is_null) {
                $this->addFunction($function, $method, $resMode, $simple);
            }
            else {
                $this->addFunction($function, $aliases[$i], $resMode, $simple);
            }
    ...
    public function addInstanceMethods($object, $class = NULL, $aliasPrefix = NULL, $resMode = HproseresMode::Normal, $simple = NULL) {
        if ($class === NULL) $class = get_class($object);
        $this->addMethods($this->getDeclaredOnlyMethods($class), $object, $aliasPrefix, $resMode, $simple);
    }
    public function addClassMethods($class, $execclass = NULL, $aliasPrefix = NULL, $resMode = HproseresMode::Normal, $simple = NULL) {
        if ($execclass === NULL) $execclass = $class;
        $this->addMethods($this->getDeclaredOnlyMethods($class), $execclass, $aliasPrefix, $resMode, $simple);
    }
    public function add() {
