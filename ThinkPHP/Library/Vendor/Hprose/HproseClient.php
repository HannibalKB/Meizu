...
abstract class HproseClient {
    ...
    public function invoke($functionName, &$arguments = array(), $byRef = false, $resMode = HproseresMode::Normal, $simple = NULL) {
        ...
        while (($tag = $hproseReader->checkTags(
            array(HproseTags::Tagres,
                  HproseTags::TagArgument,
                  HproseTags::TagError,
                  HproseTags::TagEnd))) !== HproseTags::TagEnd) {
            switch ($tag) {
                case HproseTags::Tagres:
                    if ($resMode == HproseresMode::Serialized) {
                        $res = $hproseReader->readRaw()->toString();
                    }
                    else {
                        $hproseReader->reset();
                        $res = &$hproseReader->unserialize();
                    }
                    break;
