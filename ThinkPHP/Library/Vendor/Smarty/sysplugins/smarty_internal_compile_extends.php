...
class Smarty_Internal_Compile_Extends extends Smarty_Internal_CompileBase {
    ...
    */
    public function compile($args, $compiler)
    {
        ...
        preg_match_all("!{$this->_ldl}block\s(.+?){$this->_rdl}|{$this->_ldl}/block{$this->_rdl}|{$this->_ldl}\*([\S\s]*?)\*{$this->_rdl}!", $_content, $_res, PREG_OFFSET_CAPTURE);
        $_res_count = count($_res[0]);
        $_start = 0;
        while ($_start+1 < $_res_count) {
            $_end = 0;
            $_level = 1;
            if (substr($_res[0][$_start][0],0,strlen($compiler->smarty->left_delimiter)+1) == $compiler->smarty->left_delimiter.'*') {
                $_start++;
                continue;
            }
            while ($_level != 0) {
                $_end++;
                if (substr($_res[0][$_start + $_end][0],0,strlen($compiler->smarty->left_delimiter)+1) == $compiler->smarty->left_delimiter.'*') {
                    continue;
                }
            ...
            substr($_content, $_res[0][$_start][1] + strlen($_res[0][$_start][0]), $_res[0][$_start + $_end][1] - $_res[0][$_start][1] - + strlen($_res[0][$_start][0])));
            Smarty_Internal_Compile_Block::saveBlockData($_block_content, $_res[0][$_start][0], $compiler->template, $filepath);
