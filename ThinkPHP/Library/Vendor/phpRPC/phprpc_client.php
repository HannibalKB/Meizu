 ...
 * echo "<br />";
 * // error handle
 * $res = $rpc_client->mul(1, 2);  // no mul function
 * if (is_a($res, "PHPRPC_Error")) {
 *     echo $res->toString();
 * }
 */
...
class _PHPRPC_Client {
    ...
    function invoke($funcname, &$args, $byRef = false) {
        ...
        $res = $this->_post($request);
        if (is_a($res, 'PHPRPC_Error')) {
            return $res;
        }
        $phprpc_errno = 0;
        $phprpc_errstr = NULL;
        if (isset($res['phprpc_errno'])) {
            $phprpc_errno = intval($res['phprpc_errno']);
        }
        if (isset($res['phprpc_errstr'])) {
            $phprpc_errstr = base64_decode($res['phprpc_errstr']);
        }
        $this->_warning = new PHPRPC_Error($phprpc_errno, $phprpc_errstr);
        if (array_key_exists('phprpc_output', $res)) {
            $this->_output = base64_decode($res['phprpc_output']);
        ...
        if (array_key_exists('phprpc_res', $res)) {
            if (array_key_exists('phprpc_args', $res)) {
                $arguments = unserialize($this->_decrypt(base64_decode($res['phprpc_args']), 1));
                for ($i = 0; $i < count($arguments); $i++) {
                    $args[$i] = $arguments[$i];
                }
            }
            $res = unserialize($this->_decrypt(base64_decode($res['phprpc_res']), 2));
        }
        else {
            $res = $this->_warning;
        }
    ...
    function _key_exchange() {
        ...
        if (array_key_exists('phprpc_keylen', $res)) {
            $this->_keylen = (int)$res['phprpc_keylen'];
        }
        else {
            $this->_keylen = 128;
        }
        if (array_key_exists('phprpc_encrypt', $res)) {
            $encrypt = unserialize(base64_decode($res['phprpc_encrypt']));
