...
if (extension_loaded('gmp')) {
    ...
    function bigint_str2num($str) {
        ...
        $m = '1';
        for ($i = 0; $i < $len; $i++) {
            $res = bcadd(bcmul($m, ord($str{$len - $i - 1})), $res);
            $m = bcmul($m, '256');
        }
        return $res;
    }
    ...
    function bigint_str2num($str) {
        ...
        $res = array();
        for ($i = 0; $i < $n; $i++) {
            $res[$j++] = (ord($str{$i++}) << 7) | (ord($str{$i}) >> 1);
            $res[$j++] = ((ord($str{$i++}) & 0x01) << 14) | (ord($str{$i++}) << 6) | (ord($str{$i}) >> 2);
            $res[$j++] = ((ord($str{$i++}) & 0x03) << 13) | (ord($str{$i++}) << 5) | (ord($str{$i}) >> 3);
            $res[$j++] = ((ord($str{$i++}) & 0x07) << 12) | (ord($str{$i++}) << 4) | (ord($str{$i}) >> 4);
            $res[$j++] = ((ord($str{$i++}) & 0x0f) << 11) | (ord($str{$i++}) << 3) | (ord($str{$i}) >> 5);
            $res[$j++] = ((ord($str{$i++}) & 0x1f) << 10) | (ord($str{$i++}) << 2) | (ord($str{$i}) >> 6);
            $res[$j++] = ((ord($str{$i++}) & 0x3f) << 9) | (ord($str{$i++}) << 1) | (ord($str{$i}) >> 7);
            $res[$j++] = ((ord($str{$i++}) & 0x7f) << 8) | ord($str{$i});
        }
    ...
    function bigint_random($n, $s) {
        ...
        $res = array();
        for ($i = 0; $i < $q; $i++) {
            $res[$i] = mt_rand(0, 0x7fff);
        }
        if ($r != 0) {
            $res[$q] = mt_rand(0, $lowBitMasks[$r]);
            if ($s) {
                $res[$q] |= 1 << ($r - 1);
            }
        }
