...
//		$this->_func_regexp = '[a-zA-Z_]\w*';
	...
	function _compile_tag($tag)
	{
		...
		{
			$_res = $this->_parse_variables($_match[1], $_match[2]);
			return "<?php echo $_res; ?>\n";
		}
		// process a function
		$tag_command = $_match[1][0];
		$tag_modifiers = !empty($_match[2][0]) ? $_match[2][0] : null;
		$tag_arguments = !empty($_match[3][0]) ? $_match[3][0] : null;
		$_res = $this->_parse_function($tag_command, $tag_modifiers, $tag_arguments);
		return $_res;
	}
	...
	function _parse_function($function, $modifiers, $arguments)
	{
		switch ($function) {
			...
			case 'foreach':
				...
				$_res = '<?php if (count((array)' . $_args['from'] . ')): foreach ((array)' . $_args['from'] . ' as ' . $_args['key'] . '$this->_vars[\'' . $_args['value'] . '\']): ?>';
				return $_res;
				break;
			case 'foreachelse':
			...
			case 'for':
				...
				$_res = '<?php for($for' . $this->_for_stack . ' = ' . $_args['start'] . '; ((' . $_args['start'] . ' < ' . $_args['stop'] . ') ? ($for' . $this->_for_stack . ' < ' . $_args['stop'] . ') : ($for' . $this->_for_stack . ' > ' . $_args['stop'] . ')); $for' . $this->_for_stack . ' += ((' . $_args['start'] . ' < ' . $_args['stop'] . ') ? ' . $_args['step'] . ' : -' . $_args['step'] . ')): ?>';
				if (isset($_args['value']))
				{
					$_res .= '<?php $this->assign(\'' . $this->_dequote($_args['value']) . '\', $for' . $this->_for_stack . '); ?>';
				}
				return $_res;
				break;
			case '/for':
			...
			case 'case':
				if (count($this->_switch_stack) > 0)
				{
					...
					{
						$_res .= 'switch(' . $this->_switch_stack[$_index]["var"] . '): ';
			...
			default:
				$_res = "";
				if ($this->_compile_compiler_function($function, $arguments, $_res))
				{
					return $_res;
				}
				else if ($this->_compile_custom_block($function, $modifiers, $arguments, $_res))
				{
					return $_res;
				}
				elseif ($this->_compile_custom_function($function, $modifiers, $arguments, $_res))
				{
					return $_res;
				}
				else
	...
	function _compile_compiler_function($function, $arguments, &$_res)
	{
		if ($function = $this->_plugin_exists($function, "compiler"))
		{
			$_args = $this->_parse_arguments($arguments);
			$_res = '<?php ' . $function($_args, $this) . ' ?>';
			return true;
		}
		else
		{
	...
	function _compile_custom_function($function, $modifiers, $arguments, &$_res)
	{
		...
		return compile_compile_custom_function($function, $modifiers, $arguments, $_res, $this);
	}

	function _compile_custom_block($function, $modifiers, $arguments, &$_res)
	{
		if (!function_exists('compile_compile_custom_block'))
		{
			require_once(TEMPLATE_LITE_DIR . "internal/compile.compile_custom_block.php");
		}
		return compile_compile_custom_block($function, $modifiers, $arguments, $_res, $this);
	}
			...
			switch($state) {
				...
				case 2:
					if ($value != '=')
					{
						...
						{
							$_res[$a_name] = $this->_parse_variables($_variables[1], $_variables[2]);
						}
						$state = 0;
					}
					else
	...
	function _parse_variables($variables, $modifiers)
	{
		...
		foreach($variables as $key => $value)
		{
			...
			{
				$_res .= $this->_parse_variable($tag_variable).'.';
			}
			else
			{
				$_res .= $this->_parse_modifier($this->_parse_variable($tag_variable), $modifiers[$key]).'.';
			}
		}
		return substr($_res, 0, -1);
	}
...
//			preg_match_all('/(?:[^\\\]' . $this->_dvar_regexp . '[^\\\])/', $variable, $_expand);
			...
			{
				$value = trim($value);
				$_res = str_replace($value, '" . ' . $this->_parse_variable($value) . ' . "', $_res);
			}
	...
	function _compile_variable($variable)
	{
			...
			if ($variable[0]{0} == '[' || $variable[0]{0} == '.')
			{
				...
				switch(strtoupper(str_replace($find, "", $variable[0])))
				{
					...
					case 'SECTION':
						...
						break;
					case 'LDELIM':
						$_res = "\$this->left_delimiter";
						break;
					case 'RDELIM':
						$_res = "\$this->right_delimiter";
						break;
					case 'VERSION':
						$_res = "\$this->_version";
						break;
					...
					case 'CONST':
						...
						$_res = "constant('$constant')";
						$variable = array();
						break;
					default:
						$_var_name = str_replace($find, "", $variable[0]);
						$_res = "\$this->_templatelite_vars['$_var_name']";
						break;
				}
		...
		foreach ($variable as $var)
		{
			if ($var{0} == '[')
			{
				...
				elseif ($var{0} == '$')
				{
					$_res .= "[" . $this->_compile_variable($var) . "]";
				}
				elseif ($var{0} == '#')
				{
					$_res .= "[" . $this->_compile_config($var) . "]";
				}
				else
				{
//					$_res .= "['$var']";
					...
					$_res .= "[\$this->_sections['$section']['$section_prop']]";
				}
			}
			else if ($var{0} == '.')
			{
