...
class config {
	...
	function config_load($file, $section_name = null, $var_name = null)
	{
		...
		if (preg_match("/^(.*?)(\n\[|\Z)/s", $contents, $match))
		{
			$_res["globals"] = $this->_parse_config_section($match[1]);
		}
		...
		if (preg_match_all("/^\[(.*?)\]/m", $contents, $match))
		{
			foreach ($match[1] as $section)
			{
				...
				$_res[$section] = $this->_parse_config_section($match[1]);
			}
		}
		...
		if (!empty($var_name))
		{
			...
			{
				return $_res["globals"][$var_name];
			}
			else
			{
				if(isset($_res[$section_name][$var_name]))
				{
					return $_res[$section_name][$var_name];
				}
				else
				{
					return array();
		...
		else
		{
			...
			{
				if(isset($_res[$section_name]))
				{
					return $_res[$section_name];
				}
				else
				{
					return array();
	...
	function _parse_config_section($body)
	{
		...
		for($i = 0, $for_max = count($ini[0]); $i < $for_max; $i++)
		{
			...
			if ($this->overwrite || !isset($_res[trim($keys[$i])]))
			{
				$_res[trim($keys[$i])] = $values[$i];
			}
			else
			{
				if (!is_array($_res[trim($keys[$i])]))
				{
					$_res[trim($keys[$i])] = array($_res[trim($keys[$i])]);
				}
				$_res[trim($keys[$i])][] = $values[$i];
			}
		}
