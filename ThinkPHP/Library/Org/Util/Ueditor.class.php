	...
	public function __construct($uid = ''){
		...
		switch($action){
		    ...
		    case 'uploadimage':
				...
				$res = $this->uploadFile($config, $fieldName);
				break;
			...
			case 'uploadscrawl':
				$config = array(
		            ...
		            $res=$this->uploadBase64($config,$fieldName);
		            break;
			...
			case 'catchimage':
				...
				$res = $this->saveRemote($config , $fieldName);
				break;
		...
		if (isset($_GET["callback"])) {
			if (preg_match("/^[\w_]+$/", $_GET["callback"])) {
				$this->output = htmlspecialchars($_GET["callback"]) . '(' . $res . ')';
			} else {
