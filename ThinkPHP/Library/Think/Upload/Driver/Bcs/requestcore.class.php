...
class BCS_RequestCore {
	 ...
	 * @return boolean the res of the answer
	 */
	 ...
	 * @return string The resing unparsed data from the request.
	 */
	...
	public function send_multi_request($handles, $opt = null) {
		...
		do {
			...
			while ( $done = curl_multi_info_read ( $multi_handle ) ) {
				// Since curl_errno() isn't reliable for handles that were in multirequests, we check the 'res' of the info read, which contains the curl error number, (listed here http://curl.haxx.se/libcurl/c/libcurl-errors.html )
				if ($done ['res'] > 0) {
					throw new BCS_RequestCore_Exception ( 'cURL resource: ' . ( string ) $done ['handle'] . '; cURL error: ' . curl_error ( $done ['handle'] ) . ' (' . $done ['res'] . ')' );
