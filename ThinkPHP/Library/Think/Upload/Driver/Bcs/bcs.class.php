...
class BaiduBCS {
	...
	public function upload_directory($bucket, $dir, $opt = array()) {
			...
			if (isset ( $objects [$opt ["seek_object"]] )) {
				foreach ( $objects as $object => $file ) {
					if ($object != $opt ["seek_object"]) {
						...
						//$res ['skipped'] [] = "[$num/$objectCount]  " . $file;
						$res ['skipped'] ++;
		...
		foreach ( $objects as $object => $file ) {
			...
			if (isset ( $opt [self::IMPORT_BCS_PRE_FILTER] ) && function_exists ( $opt [self::IMPORT_BCS_PRE_FILTER] )) {
				...
				if ($bolRes !== true) {
					...
					//$res ['skipped'] [] = "id[$num/$objectCount]object[$object]file[$file]";
					$res ['skipped'] ++;
					$num ++;
					continue;
				}
			...
			if ($response->isOK ()) {
				...
				$res ["failed"] [] = "id[$num/$objectCount]object[$object]file[$file]";
				$this->log ( "Upload Failed id[$num/$objectCount]object[$object]file[$file].", $opt );
			}
		...
		$res_str .= "**********************res Summary**********************\r\n";
		$res_str .= "**********************************************************\r\n";
		$res_str .= "Upload directory :  [$dir]\r\n";
		$res_str .= "File num :  [$objectCount]\r\n";
		$res_str .= "Success: \r\n\tNum: " . $res ["success"] . "\r\n";
		$res_str .= "Skipped:\r\n\tNum:" . $res ["skipped"] . "\r\n";
		//		foreach ( $res ["skipped"] as $skip ) {
		//			$res_str .= "\t$skip\r\n";
		//		}
		$res_str .= "Failed:\r\n\tNum:" . count ( $res ["failed"] ) . "\r\n";
		foreach ( $res ["failed"] as $fail ) {
			$res_str .= "\t$fail\r\n";
		}
		if (isset ( $opt [self::IMPORT_BCS_LOG_METHOD] )) {
			$this->log ( $res_str, $opt );
		} else {
			echo $res_str;
		}
		return $res;
	...
	private function analyze_user_acl($acl) {
		...
		if (is_array ( $acl )) {
			//(1).a
			$res ['content'] = $this->check_user_acl ( $acl );
