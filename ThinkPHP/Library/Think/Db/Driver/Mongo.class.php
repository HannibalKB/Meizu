...
class Mongo extends Driver {
    ...
    public function command($command=array(), $options=array()) {
        ...
        try{
            ...
            $this->debug(true);
            $res   = $this->_mongo->command($command);
            $this->debug(false);
            
            if($cache && $res['ok']) { // 查询缓存写入
                S($key,$res,$cache['expire'],$cache['type']);
            }
            return $res;
    ...
    public function execute($code,$args=array()) {
        ...
        $res   = $this->_mongo->execute($code,$args);
        $this->debug(false);
        if($res['ok']) {
            return $res['retval'];
        }else{
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        try{
            $this->debug(true);
            $res =  $replace?   $this->_collection->save($data):  $this->_collection->insert($data);
            $this->debug(false);
            if($res) {
               $_id    = $data['_id'];
                if(is_object($_id)) {
    ...
    public function insertAll($dataList,$options=array()) {
        ...
        try{
            $this->debug(true);
            $res =  $this->_collection->batchInsert($dataList);
            $this->debug(false);
            return $res;
        } catch (\MongoCursorException $e) {
            E($e->getMessage());
    ...
    public function getMongoNextId($pk) {
        ...
        try{
            $this->debug(true);
            $res   =  $this->_collection->find(array(),array($pk=>1))->sort(array($pk=>-1))->limit(1);
            $this->debug(false);
        } catch (\MongoCursorException $e) {
            E($e->getMessage());
        }
    ...
    public function update($data,$options) {
        ...
        try{
            ...
            $res   = $this->_collection->update($query,$set,$multiple);
            $this->debug(false);
            return $res;
        } catch (\MongoCursorException $e) {
            E($e->getMessage());
    ...
    public function delete($options=array()) {
        ...
        try{
            $this->debug(true);
            $res   = $this->_collection->remove($query);
            $this->debug(false);
            return $res;
        } catch (\MongoCursorException $e) {
            E($e->getMessage());
    ...
    public function select($options=array()) {
        ...
        try{
            ...
            $resSet  =  iterator_to_array($_cursor);
            return $resSet;
        } catch (\MongoCursorException $e) {
            E($e->getMessage());
        }
