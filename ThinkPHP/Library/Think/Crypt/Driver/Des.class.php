...
class Des {
    ...
    private static function _des($key, $message, $encrypt, $mode=0, $iv=null) {
      ...
      while ($m < $len) {
        ...
        //for Cipher Block Chaining mode, xor the message with the previous res
        ...
        for ($j=0; $j<$iterations; $j+=3) {
          ...
          for ($i=$looping[$j]; $i!=$endloop; $i+=$loopinc) { //for efficiency
            ...
            //the res is attained by passing these bytes through the S selection functions
            $temp = $left;
            $left = $right;
        ...
        $tempres .= (chr($left>>24 & $masks[24]) . chr(($left>>16 & $masks[16]) & 0xff) . chr(($left>>8 & $masks[8]) & 0xff) . chr($left & 0xff) . chr($right>>24 & $masks[24]) . chr(($right>>16 & $masks[16]) & 0xff) . chr(($right>>8 & $masks[8]) & 0xff) . chr($right & 0xff));

        $chunk += 8;
        if ($chunk == 512) {$res .= $tempres; $tempres = ""; $chunk = 0;}
      } //for every 8 characters, or 64 bits in the message

      //return the res as an array
      return ($res . $tempres);
    } //end of des
