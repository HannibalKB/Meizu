...
class RelationModel extends Model {
    ...
    protected function _after_find(&$res,$options) {
        ...
        if(!empty($options['link']))
            $this->getRelation($res,$options['link']);
    }

    // 查询数据集成功后的回调方法
    protected function _after_select(&$res,$options) {
        // 获取关联数据 并附加到结果中
        if(!empty($options['link']))
            $this->getRelations($res,$options['link']);
    }
     ...
     * 获取返回数据集的关联记录
     * @access protected
     * @param array $resSet  返回数据
     * @param string|array $name  关联名称
     * @return array
     */
    protected function getRelations(&$resSet,$name='') {
        // 获取记录集的主键列表
     ...
     * @return array
     */
    protected function getRelation(&$res,$name='',$return=false) {
        if(!empty($this->_link)) {
                            ...
                            if(isset($val['as_fields']) && in_array($mappingType,array(self::HAS_ONE,self::BELONGS_TO)) ) {
                                ...
                                foreach ($fields as $field){
                                    if(strpos($field,':')) {
                                        ...
                                        $res[$nick]  =  $relationData[$relationName];
                                    }else{
                                        $res[$field]  =  $relationData[$field];
                                    }
                                }
                            }else{
                                $res[$mappingName] = $relationData;
                            }
                            unset($relationData);
                        }else{
                            return $relationData;
                            ...
                            switch($mappingType) {
                                case self::HAS_ONE:
                                    switch (strtoupper($opType)){
                                        ...
                                        $res   =  $model->add($mappingData);
                                        break;
                                        case 'SAVE':    // 更新关联数据
                                        $res   =  $model->where($mappingCondition)->save($mappingData);
                                        break;
                                        case 'DEL': // 根据外键删除关联数据
                                        $res   =  $model->where($mappingCondition)->delete();
                                        break;
                                    }
                                    break;
                                case self::BELONGS_TO:
                                    ...
                                    switch (strtoupper($opType)){
                                        ...
                                        foreach ($mappingData as $vo){
                                            if(isset($vo[$pk])) {// 更新数据
                                                ...
                                                $res   =  $model->where($mappingCondition)->save($vo);
                                            }else{ // 新增数据
                                ...
                                case self::MANY_TO_MANY:
                                    ...
                                    switch (strtoupper($opType)){
                                        ...
                                        case 'DEL': // 根据外键删除中间表关联数据
                                            $res =   $this->table($mappingRelationTable)->where($mappingCondition)->delete();
                                            break;
                                    }
                                    break;
                            }
