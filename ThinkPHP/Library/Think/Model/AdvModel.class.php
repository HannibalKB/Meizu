...
class AdvModel extends Model {
    ...
    // 查询成功后的回调方法
    protected function _after_find(&$res,$options='') {
        // 检查序列化字段
        $this->checkSerializeField($res);
        // 获取文本字段
        $this->getBlobFields($res);
        // 检查字段过滤
        $res   =  $this->getFilterFields($res);
        // 缓存乐观锁
        $this->cacheLockVersion($res);
    }

    // 查询数据集成功后的回调方法
    protected function _after_select(&$resSet,$options='') {
        // 检查序列化字段
        $resSet   =  $this->checkListSerializeField($resSet);
        // 获取文本字段
        $resSet   =  $this->getListBlobFields($resSet);
        // 检查列表字段过滤
        $resSet   =  $this->getFilterListFields($resSet);
    }
     ...
     * @return mixed
     */
    public function returnres($data,$type='') {
        if('' === $type)
     ...
     * 获取数据的时候过滤数据字段
     * @access protected
     * @param mixed $res 查询的数据
     * @return array
     */
    protected function getFilterFields(&$res) {
                ...
                if(isset($res[$field])) {
                    ...
                    if(!empty($fun)) {
                        if(isset($filter[2]) && $filter[2]){
                            // 传递整个数据对象作为参数
                            $res[$field]  =  call_user_func($fun,$res);
                        }else{
                            // 传递字段的值作为参数
                            $res[$field]  =  call_user_func($fun,$res[$field]);
                        }
                    }
                }
            }
    ...
    protected function getFilterListFields(&$resSet) {
        if(!empty($this->_filter)) {
            foreach ($resSet as $key=>$res)
                $resSet[$key]  =  $this->getFilterFields($res);
        }
        return $resSet;
    }
     ...
     * 返回数据列表
     * @access protected
     * @param array $resSet 数据
     * @param string $type 返回类型 默认为数组
     * @return void
     */
    protected function returnresSet(&$resSet,$type='') {
        foreach ($resSet as $key=>$data)
            $resSet[$key]  =  $this->returnres($data,$type);
        return $resSet;
    }
     ...
     * 获取数据集的文本字段
     * @access protected
     * @param mixed $resSet 查询的数据
     * @param string $field 查询的字段
     * @return void
     */
    protected function getListBlobFields(&$resSet,$field='') {
        if(!empty($this->blobFields)) {
            foreach ($resSet as $key=>$res){
                $res =   $this->getBlobFields($res,$field);
                $resSet[$key]    =   $res;
            }
        }
        return $resSet;
    ...
    // 检查返回数据的序列化字段
    protected function checkSerializeField(&$res) {
        // 检查序列化字段
        if(!empty($this->serializeField)) {
            foreach ($this->serializeField as $key=>$val){
                if(isset($res[$key])) {
                    $serialize   =   unserialize($res[$key]);
                    foreach ($serialize as $name=>$value)
                        $res[$name]  =   $value;
                    unset($serialize,$res[$key]);
                }
            }
        }
        return $res;
    ...
    // 检查数据集的序列化字段
    protected function checkListSerializeField(&$resSet) {
        // 检查序列化字段
        if(!empty($this->serializeField)) {
            foreach ($this->serializeField as $key=>$val){
                foreach ($resSet as $k=>$res){
                    if(isset($res[$key])) {
                        $serialize   =   unserialize($res[$key]);
                        foreach ($serialize as $name=>$value)
                            $res[$name]  =   $value;
