...
class Model {
    ...
    // 链操作方法列表
    protected $methods          =   array('strict','order','alias','having','group','lock','distinct','auto','filter','validate','res','token','index','force');
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        // 写入数据到数据库
        $res = $this->db->insert($data,$options,$replace);
        if(false !== $res && is_numeric($res)) {
            $pk     =   $this->getPk();
              // 增加复合主键支持
            if (is_array($pk)) return $res;
    ...
    public function addAll($dataList,$options=array(),$replace=false){
        ...
        $options =  $this->_parseOptions($options);
        // 写入数据到数据库
        $res = $this->db->insertAll($dataList,$options,$replace);
        if(false !== $res ) {
            $insertId   =   $this->getLastInsID();
            if($insertId) {
                return $insertId;
    ...
    public function selectAdd($fields='',$table='',$options=array()) {
        ...
        $options =  $this->_parseOptions($options);
        // 写入数据到数据库
        if(false === $res = $this->db->selectInsert($fields?:$options['field'],$table?:$this->getTableName(),$options)){
            // 数据库插入操作失败
    ...
    public function save($data='',$options=array()) {
        ...
        $res     =   $this->db->update($data,$options);
        if(false !== $res && is_numeric($res)) {
    ...
    public function select($options=array()) {
        ...
        $resSet  = $this->db->select($options);
        if(false === $resSet) {
            return false;
        }
        if(!empty($resSet)) { // 有查询结果
            if(is_string($resSet)){
                return $resSet;
            }

            $resSet  =   array_map(array($this,'_read_data'),$resSet);
            $this->_after_select($resSet,$options);
            if(isset($options['index'])){ // 对数据集进行索引
                $index  =   explode(',',$options['index']);
                foreach ($resSet as $res){
                    $_key   =  $res[$index[0]];
                    if(isset($index[1]) && isset($res[$index[1]])){
                        $cols[$_key] =  $res[$index[1]];
                    }else{
                        $cols[$_key] =  $res;
                    }
                }
    ...
    public function find($options=array()) {
        ...
        // 读取数据后的处理
        $data   =   $this->_read_data($resSet[0]);
        $this->_after_find($data,$options);
        if(!empty($this->options['res'])) {
            return $this->returnres($data,$this->options['res']);
        }
        $this->data     =   $data;
        if(isset($cache)){
            S($key,$data,$cache);
    ...
    protected function returnres($data,$type=''){
        if ($type){
            if(is_callable($type)){
